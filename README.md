...
**만들어진 것 (0단계 + 1단계)**
- 지도가 상단에 뜨고, 회사 위치 마커 + 실제 식당 마커 표시
- 카테고리 필터 탭 (한식/일식/중식/양식/브런치/카페) - 클릭하면 리스트가 바뀜
- **카카오 로컬 API로 독막로 331 반경 1km 내 식당을 실시간으로 자동 수집** (더미 데이터 아님)
- 거리(m) + 도보 예상 시간(분) 자동 계산해서 리스트/지도에 표시
- 대학내일 브랜드 컬러(네이비 + 틸) 적용

**아직 없는 것 (다음 단계에서 추가 예정)**
- 가격대/인원수용/웨이팅 정보 (카카오 API가 제공하지 않는 값이라 관리자 입력 또는 후기 집계로 채울 예정 - 지금은 "정보 없음"으로 표시됨)
...
이번 업데이트부터는 더미 데이터가 아니라, 사이트에 접속할 때마다 `app/api/restaurants/route.js`가 서버에서 카카오 로컬 API(`KAKAO_REST_API_KEY` 사용)를 호출해서 독막로 331 기준 반경 1km 내 한식/일식/중식/양식/브런치/카페를 실시간으로 검색해 옵니다.

- 카카오 API는 가격대/인원수용/웨이팅 정보를 제공하지 않기 때문에, 이 3개 항목은 현재 "정보 없음"으로 표시됩니다. 다음 단계(후기 기능)에서 실제 값으로 채워질 예정입니다.
- **주의**: 이 기능은 `KAKAO_REST_API_KEY` 환경변수가 Vercel에 반드시 등록되어 있어야 동작합니다. (지도용 `NEXT_PUBLIC_KAKAO_JS_KEY`와는 다른 키입니다.)
