...
// 특정 식당의 후기들에서 실제로 언급된 웨이팅/인원수/추천대상/가격대 값들을 전부 모음 (필터링에 사용)
// summarizeReviews의 "top" 값과 달리, 한 번이라도 언급된 값이면 전부 포함합니다.
export function getRestaurantFilterData(restaurantId) {
  ...
  reviews.forEach((review) => {
    (Array.isArray(review.waiting) ? review.waiting : [review.waiting]).forEach(
      (v) => v && waitingSet.add(v)
    );
    (Array.isArray(review.headcount) ? review.headcount : [review.headcount]).forEach(
      (v) => v && headcountSet.add(v)
    );
    (Array.isArray(review.recommendedFor) ? review.recommendedFor : [review.recommendedFor]).forEach(
      (v) => v && recommendedForSet.add(v)
    );
