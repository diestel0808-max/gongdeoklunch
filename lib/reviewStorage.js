...
// 특정 식당의 후기들에서 실제로 언급된 웨이팅/동행/가격대 값들을 전부 모음 (필터링에 사용)
...
export function getRestaurantFilterData(restaurantId) {
  ...
  reviews.forEach((review) => {
    ...
    (Array.isArray(review.companion) ? review.companion : [review.companion]).forEach(
      (v) => v && companionSet.add(v)
    );
    if (review.priceRange) priceRangeSet.add(review.priceRange);
  });

  return { waitingSet, companionSet, priceRangeSet, reviewCount: reviews.length };
}
